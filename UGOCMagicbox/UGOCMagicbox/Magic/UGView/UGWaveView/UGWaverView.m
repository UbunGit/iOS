...
@property (nonatomic, strong) NSMutableArray<UGWave*> *waves;
...
- (void)layoutSubviews {
    ...
    if (self.layer.sublayers.count > 0) {
        ...
        [_waves enumerateObjectsUsingBlock:^(UGWave * _Nonnull wave, NSUInteger idx, BOOL * _Nonnull stop) {
            if (wave.layer) {
...
- (void)start {
    if (!_displayLink) {
        self.displayLink = [CADisplayLink displayLinkWithTarget:[UGWaveWeakProxy proxyWithTarget:self] selector:@selector(drawWave:)];
 ...
 */
- (UIBezierPath *)createWavePath:(UGWaveStyle)style
    ...
    for (CGFloat x = 0.0f; x <= viewWidth;) {
        CGFloat y = 0.0f;
        if (style == UGWaveStyleSin) {
            if (direction == UGWaveDirectionLeft) {
...
@interface UGWaveWeakProxy ()

@property (nonatomic, weak, readonly) id target;

@end

@implementation UGWaveWeakProxy

+ (instancetype)proxyWithTarget:(id)target {
    return [[UGWaveWeakProxy alloc] initWithTarget:target];
}
