...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"选择" style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(cancelButtonClick)];
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.photoPickerBottomToolBarConfigBlock) {
        self.bottomToolBar = tzImagePickerVc.photoPickerBottomToolBarConfigBlock(self);
    }
...
-(void)allSelect:(BOOL)isselctAll{
    ...
    for (TZAssetModel *model in _models) {
        if (!isselctAll) {
            ...
            if (tzImagePickerVc.showSelectedIndex || tzImagePickerVc.showPhotoCannotSelectLayer) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"TZ_PHOTO_PICKER_RELOAD_NOTIFICATION" object:weakSelf.navigationController];
            }
